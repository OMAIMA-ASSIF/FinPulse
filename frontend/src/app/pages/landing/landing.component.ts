...
import { faChartLine, faRobot, faBolt, faBell, faBookmark, faSearch, faChartSimple, faShield } from '@fortawesome/free-solid-svg-icons';
...
export class LandingComponent implements AfterViewInit, OnDestroy {
  ...
  icons = {
    ...
    strategy: faBolt,
    alerts: faBell,
    watchlist: faBookmark,
    discover: faSearch,
    charts: faChartSimple,
    risk: faShield
  };

  features = [
    { icon: this.icons.dashboard, title: 'Live NCI Dashboard', desc: 'Real-time narrative consistency scoring for hundreds of companies.' },
    { icon: this.icons.agent,     title: 'AI Agent', desc: 'Ask questions and get instant NCI analysis, risks and SEC insights.' },
    { icon: this.icons.strategy,  title: 'Strategy Detection', desc: 'AI auto-detects investment theses and generates strategy reports.' },
    { icon: this.icons.alerts,    title: 'Smart Alerts', desc: 'Get notified when NCI drops, sentiment shifts, or risks emerge.' },
    { icon: this.icons.watchlist, title: 'Watchlist', desc: 'Pin companies and track narrative evolution over time.' },
    { icon: this.icons.discover,  title: 'Discover & Compare', desc: 'Filter by sector, sort by NCI, and compare companies side by side.' },
    { icon: this.icons.charts,    title: 'NCI History Charts', desc: 'Premium financial charts with 1M/3M/6M historical NCI trends.' },
    { icon: this.icons.risk,      title: 'SEC Risk Detection', desc: 'Detect narrative contradictions against SEC filings automatically.' }
  ];
