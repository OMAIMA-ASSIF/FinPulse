...
export class CompanyDetailComponent implements OnInit, OnDestroy {
  ...
  sentimentColor(s: number): string {
    return s > 0.05 ? 'var(--green)' : s < -0.05 ? 'var(--red)' : 'var(--orange)';
  }
  ...
  sentimentClass(s: number): string { return s > 0.05 ? 'chip chip-green' : s < -0.05 ? 'chip chip-red' : 'chip chip-amber'; }

  newsClass(s: number): string { return s > 0.05 ? 'chip-green' : s < -0.05 ? 'chip-red' : 'chip-amber'; }
  newsLabel(s: number): string { return s > 0.05 ? 'POS' : s < -0.05 ? 'NEG' : 'NEU'; }
}
