...
import {
  ...
  PageResponse, CreateStrategyRequest, RiskExplanation, Anomaly
} from '../models';
...
export class ApiService {
  ...
  getIntegrationStatus(): Observable<IntegrationStatus> {
    return this.http.get<IntegrationStatus>(`${this.base}/integration/status`);
  }
  ...
  getRiskExplanation(ticker: string): Observable<RiskExplanation> {
    return this.http.get<RiskExplanation>(`${this.base}/companies/ticker/${ticker}/risk-explanation`);
  }

  getAnomalies(ticker: string, filingId?: number, topK = 5): Observable<{ paragraphs: Anomaly[] }> {
    // Call data-ingestion service through proxy to avoid CORS
    let params = new HttpParams().set('top_k', topK);
    if (filingId) params = params.set('filing_id', filingId);
    return this.http.get<{ paragraphs: Anomaly[] }>(`/data-ingestion/api/v1/embeddings/${ticker}/anomalies`, { params });
  }
