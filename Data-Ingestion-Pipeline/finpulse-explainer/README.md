...
3. Copier `.env.example` vers `.env` à la racine de `FinPulse` et définir `MISTRAL_API_KEY` (ou une autre clé selon le
   modèle choisi).
...
**Erreur fréquente :** `Mistral API key must be set` — la clé doit être dans `FinPulse/.env` (racine du projet, pas dans
`spring-ai-service/`).
