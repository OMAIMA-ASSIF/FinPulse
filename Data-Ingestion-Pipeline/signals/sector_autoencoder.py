...
def compute_embeddings_anomaly_scores(
    ...
    for embedding, arr in zip(embeddings, embedding_arrays):
        ...
        # Normaliser en anomaly_score [0, 1] avec sigmoid pour meilleure discrimination
        # score → 0.5 si mse ≈ threshold, score → 1 si mse >> threshold, score → 0 si mse << threshold
        if threshold > 0:
            # Sigmoid avec k=5 pour transition rapide autour du threshold
            # anomaly_score = 1 / (1 + exp(-k * (mse - threshold)))
            k = 5.0
            anomaly_score = float(1.0 / (1.0 + np.exp(-k * (mse - threshold))))
        else:
            anomaly_score = 0.0
