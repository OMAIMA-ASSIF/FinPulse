...
def _normalize_llm_payload(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(explanation, str):
        nested = _extract_json_from_text(explanation)
        if nested:
            return nested
...
class ExplicabilityEngine:
    ...
    def _step4_generate_prompt(
        ...
        return {
            ...
            "companyName": filing_data["company_name"],
            "sector": sector_profile.sector_name,
            "sicCode": sector_profile.sector_code,
            "fiscalYear": filing_data.get("fiscal_year") or 2023,
            "nci_global": nci,
            "signals": {s: d["value"] for s, d in filing_data.get("signals", {}).items() if d.get("value") is not None},
            "anomalies": [
                {"text": a["text"], "anomalyScore": a["anomaly_score"], "section": "Unknown"} 
                for a in filing_data.get("top_anomalous_paragraphs", [])
            ],
            "sectorBenchmarks": {
                k: v for k, v in {
                    "nci_global": sector_profile.avg_nci,
                    "rlds": sector_profile.avg_rlds,
                    "fundamental_deterioration": sector_profile.avg_fundamental,
                    "insider_signal": sector_profile.avg_insider,
                    "market_signal": sector_profile.avg_market,
    ...
    def _step6_parse_response(
        ...
        try:
            ...
            # Spring Boot might return the full text in "explanation"
            summary_text = content.get("summary") or content.get("explanation") or ""
