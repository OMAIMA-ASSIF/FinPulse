...
public class StrategyService {
    ...
    public StrategyDTO createStrategy(Long userId, StrategyRequestDTO request) {
        ...
        Company company = companyService.findEntityByP1Id(request.getCompanyId().longValue());

        if (strategyRepository.existsByUserIdAndCompanyId(userId, company.getId())) {
        ...
        try {
            watchlistService.pin(userId, company.getIngestionCompanyId());
