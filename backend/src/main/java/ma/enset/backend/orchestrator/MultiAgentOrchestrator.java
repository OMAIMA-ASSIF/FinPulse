...
/**
 * Orchestrateur multi-agents principal pour FinPulse.
 * Gère l'extraction de tickers, la détection d'intention, et l'orchestration des 5 agents.
 */
@Service
@Slf4j
...
public class MultiAgentOrchestrator {
    ...
    public OrchestratorResult handleMessage(User user, String ticker, String userMessage, String conversationId) {
        ...
        if (ticker == null || ticker.isBlank()) {
            ...
            if (extracted == null && session.getCompanyTicker() != null
                ...
                log.info("Utilisation du ticker de la session comme fallback : {}", extracted);
            }
        ...
        // 3. Mettre à jour le ticker courant de la session
    ...
    private IntentResult detectIntentWithLLM(String message, String ticker) {
        try {
            ...
            if (start >= 0 && end > start) {
                var node = objectMapper.readTree(response.substring(start, end));
    ...
    public boolean checkIfCompanyExists(String ticker) {
        boolean exists = ingestionPipelineService.companyExists(ticker);
        log.info("Company check for {}: {}", ticker, exists ? "found" : "missing");
        return exists;
    }
    ...
    public record OrchestratorResult(
        ...
        public static OrchestratorResult chatResponse(String text, String conversationId) {
            return new OrchestratorResult("CHATBOT", text, null, conversationId);
        }

        public static OrchestratorResult clarification(String text, String conversationId) {
            return new OrchestratorResult("CLARIFICATION", text, null, conversationId);
        }

        public static OrchestratorResult reportGenerated(ReportResult report) {
            return new OrchestratorResult("REPORT", null, report, null);
        }
    }
}
